...
const Admin = {
  ...
  showAddWorkerScreen() {
    ...
    document.getElementById('aw-save-btn').addEventListener('click', async () => {
      ...
      const [bg, fg] = avColors[all.length % avColors.length];
      ...
      App.showToast(name + ' added successfully!');
  ...
  async editWorker(workerId) {
    ...
    App.showDialog(
      w.name,
      `Mobile: ${w.mobile} | Category: ${w.category} | Status: ${w.isActive ? 'Active' : 'Inactive'}`,
      [
        ...
        { label: 'Reset PIN', class: 'btn-outline', action: () => {
            App.showInputDialog('Reset PIN', `Enter new 4-6 digit PIN for ${w.name}:`, 'New PIN', async (val) => {
  ...
  async renderTemplates() {
    ...
    cats.forEach(cat => {
      const ct = active.filter(t => t.category === cat);
      if (!ct.length) return;
  ...
  showTemplateEditor(templateId) {
    ...
    this._renderFieldsList = () => {
      ...
      if (!fl) return;
      if (!this._tempFields.length) { fl.innerHTML = '<div style="color:var(--ink3);font-size:13px;text-align:center;padding:8px">No fields yet — add below</div>'; return; }
      fl.innerHTML = this._tempFields.map((f, i) => `
    ...
    document.getElementById('save-tpl-btn').addEventListener('click', async () => {
      ...
      await DB.put('templates', { id, name, category: cat, icon, borderColor: borders[cat], fields: [...this._tempFields], isDeleted: false, createdAt: new Date().toISOString() });
  ...
  addField(type) {
    ...
    const labels = { checkbox:'Checkbox label', text:'Text field label', number:'Number field label', dropdown:'Dropdown label', image:'Photo label', date:'Date field label' };
    App.showInputDialog('Add ' + type + ' field', 'Enter the field label:', labels[type] || 'Field label', (label) => {
      if (!label || !label.trim()) return;
      const field = { id: 'f' + Date.now(), type, label: label.trim(), required: false };
      if (type === 'dropdown') {
        App.showInputDialog('Dropdown options', 'Enter options separated by comma:', 'Option 1,Option 2,Option 3', (opts) => {
          field.options = (opts || 'Option 1,Option 2').split(',').map(o => o.trim()).filter(Boolean);
  ...
  renderAssignStep(step) {
    ...
    const ui = document.getElementById('assign-steps-ui');
    ui.innerHTML = [1,2,3,4].map((s, i) => `
      ${i > 0 ? `<div class="step-line ${step > s - 1 ? 'done' : ''}"></div>` : ''}
      <div class="step-dot ${step > s ? 'done' : step === s ? 'current' : 'upcoming'}">${step > s ? '✓' : s}</div>`).join('');
  ...
  async renderAssignStep1(content, footer) {
    ...
    content.querySelectorAll('.selectable').forEach(el => {
      el.addEventListener('click', () => {
        ...
        const c = document.getElementById('tpl-sel-count');
  ...
  async renderAssignStep2(content, footer) {
    ...
    content.querySelectorAll('.selectable').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (c) c.textContent = sel.length + ' workers selected';
      });
    });
  ...
  selectWorkerGroup(cat) {
    const grp = this._assignWorkers.filter(w => w.category === cat);
    grp.forEach(w => { if (!this.assignState.workers.includes(w.id)) this.assignState.workers.push(w.id); });
    document.querySelectorAll('#worker-sel-list .selectable').forEach(el => {
      if (grp.find(w => w.id === el.dataset.workerId)) { el.classList.add('selected'); el.querySelector('input').checked = true; }
    });
    const c = document.getElementById('wk-sel-count');
    if (c) c.textContent = this.assignState.workers.length + ' workers selected';
  },
  ...
  renderAssignStep3(content, footer) {
    ...
    const freqs = ['Daily','Weekly','Monthly','Ad-hoc','Once'];
    content.innerHTML = `
      ...
      <div class="field"><label class="field-label">Frequency</label>
        <div class="freq-grid">
          ${freqs.map(f => `<button class="freq-btn ${s.frequency===f?'active':''}" onclick="adminSetFreq('${f}',this)">${f}</button>`).join('')}
        </div></div>
  ...
  async renderAssignStep4(content, footer) {
    ...
    content.innerHTML = `
      ...
      <div class="card" style="background:#ECFDF5;border-color:#6EE7B7">
        <div style="font-size:12px;font-weight:700;color:var(--emerald);margin-bottom:12px;letter-spacing:.04em">ASSIGNMENT SUMMARY</div>
        ...
        <div class="review-row"><span class="review-label">End</span><span class="review-value">${s.endDate || 'Open-ended'}</span></div>
      ...
      <div class="card">
        <div style="font-size:12px;font-weight:600;margin-bottom:8px">Templates selected</div>
        ${templates.map(t => t ? `<div style="font-size:13px;padding:5px 0;border-bottom:1px solid var(--line)">${t.icon} ${t.name}</div>` : '').join('')}
      </div>
      <div class="card">
        <div style="font-size:12px;font-weight:600;margin-bottom:8px">Workers assigned</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
    ...
    for (let d = 0; d < iters; d++) {
      ...
      for (const tpl of templates) {
        ...
        for (const worker of workers) {
          ...
          await DB.put('tasks', { id: taskId, workerId: worker.id, templateId: tpl.id, templateName: tpl.name, templateIcon: tpl.icon, category: tpl.category, date: dateStr, dueTime: s.dueTime, status: 'pending', communityId: worker.communityId, assignedAt: new Date().toISOString() });
          created++;
        }
      }
    }
  ...
  async renderSettings() {
    ...
    const allSubs    = (await DB.getAll('submissions')).length;
    const syncedSubs = (await DB.getAll('submissions')).filter(s => s.synced).length;
    ...
    content.innerHTML = `
      ...
      <div class="card">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
          ...
          <div>
            ...
            <div style="font-size:12px;color:var(--ink3);margin-top:2px">Paste your deployed Apps Script URL here once — never edit code again</div>
          </div>
        </div>
        <div class="field">
          ...
          <input type="url" class="field-input" id="sheets-url-input"
            ...
            value="${savedUrl}"
            style="font-size:12px;font-family:var(--fm)">
          <div style="font-size:11px;color:var(--ink3);margin-top:5px">
            Get this from: Google Sheet → Extensions → Apps Script → Deploy → Manage Deployments → Copy URL
          </div>
        </div>
        <div style="display:flex;gap:8px">
      ...
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-number" style="color:var(--emerald)">${syncedSubs}</div>
          <div class="stat-label">Synced to Sheets</div>
        </div>
        <div class="stat-card">
          <div class="stat-number" style="color:${queueItems > 0 ? 'var(--amber)' : 'var(--emerald)'}">${queueItems}</div>
      ...
      <div class="card" style="border-left:3px solid var(--amber);border-radius:0 12px 12px 0">
        ...
        <div style="font-size:12px;color:var(--ink3);margin-bottom:10px">These will upload automatically when the Sheets URL is saved and the device is online.</div>
      ...
      <div class="card" style="font-size:13px;line-height:1.9;color:var(--ink2)">
        1. You deploy the Apps Script <b>once</b> from your Google Sheet<br>
        2. Paste the URL here — <b>never touch code again</b><br>
        3. Every task submission automatically appends a row to your sheet<br>
        4. If offline, submissions queue locally and sync when back online<br>
      ...
      <div class="card">
        ...
        <div class="field" style="margin-bottom:0">
          <label class="field-label">Community ID</label>
    ...
    document.getElementById('save-sheets-url-btn').addEventListener('click', async () => {
      ...
      if (!url.startsWith('https://script.google.com')) {
        status.style.color = 'var(--rose)';
        status.textContent = '⚠ URL must start with https://script.google.com';
        return;
      }
    ...
    // Logout
    document.getElementById('admin-logout-settings').addEventListener('click', () => {
      Auth.logout(); App.navigate('login');
    });
  },

// ── Global bridge functions for onclick= in innerHTML ─────────────────────────
